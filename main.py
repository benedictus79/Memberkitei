...
from utils import format_url, os, clear_folder_name, create_folder, parsear_html, shorten_folder_name
...
def get_lesson_desc(soup,lesson_folder, lesson_title):
  div_tag = soup.find('div', class_ = 'mt-8 grid gap-8 lg:grid-cols-3')
  if div_tag:
    parsear_html(div_tag, lesson_folder, lesson_title)
...
def find_and_download_video(lesson_link, lesson_folder, lesson_title):
  ...
  get_lesson_desc(soup, lesson_folder, lesson_title) 
